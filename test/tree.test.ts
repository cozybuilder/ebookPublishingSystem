/** 통합 스프린트 21 — Tree (:::tree). 실행: npm run test:tree */
...
console.log('Tree 테스트 실행\n');
const md = '# 책\n\n## Chapter 1. 가\n\n:::tree\n- 프로젝트\n  - 디자인\n  - 개발\n- 문서\n:::\n';
const b = parseBook(md).chapters[0].blocks[0] as { type: string; items: { depth: number; label: string }[] };
check('파서: tree depth(들여쓰기)', b.type === 'tree' && b.items.length === 4 && b.items[0].depth === 0 && b.items[1].depth === 1);
const html = toHtml(md);
check('HTML: tree + depth padding', html.includes('class="tree"') && html.includes('class="tree-row"') && html.includes('padding-left:18px'));
const card = { type: 'TreeCard', items: [{ depth: 0, label: 'r' }, { depth: 1, label: 'c' }] } as never;
check('EPUB/DOCX: 렌더', componentToXhtml(card, { media: [] } as never).includes('tree-row') && componentToXml(card).includes('└'));
