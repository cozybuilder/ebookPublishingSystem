/** 통합 스프린트 22 — Pagination (:::pagination). 실행: npm run test:pagination */
...
console.log('Pagination 테스트 실행\n');
const md = '# 책\n\n## Chapter 1. 가\n\n:::pagination\ncurrent: 2\ntotal: 5\n:::\n';
const b = parseBook(md).chapters[0].blocks[0] as { type: string; current: number; total: number };
check('파서: pagination current/total', b.type === 'pagination' && b.current === 2 && b.total === 5);
check('파서: current clamp(>total → total)', (parseBook('# 책\n\n## Chapter 1. 가\n\n:::pagination\ncurrent: 9\ntotal: 5\n:::\n').chapters[0].blocks[0] as { current: number }).current === 5);
const html = toHtml(md);
check('HTML: pgn dots + 현재 강조 + 정보', html.includes('class="pgn"') && html.includes('pgn-dot on') && html.includes('2 / 5 페이지'));
const card = { type: 'PaginationCard', current: 1, total: 3 } as never;
check('EPUB/DOCX: 렌더', componentToXhtml(card, { media: [] } as never).includes('pgn') && componentToXml(card).includes('페이지'));
check('안전: 기본값(total>=1)', (parseBook('# 책\n\n## Chapter 1. 가\n\n:::pagination\n:::\n').chapters[0].blocks[0] as { total: number }).total === 1);
