/** 통합 스프린트 19 — Tag Group (:::tags). 실행: npm run test:tags */
...
console.log('Tag Group 테스트 실행\n');
const md = '# 책\n\n## Chapter 1. 가\n\n:::tags\n- AI 영상\n- 초보자\n- 무료\n:::\n';
const b = parseBook(md).chapters[0].blocks[0] as { type: string; items: string[] };
check('파서: tags 불릿 3개', b.type === 'tags' && b.items.length === 3 && b.items[0] === 'AI 영상');
check('파서: 쉼표 구분도 허용', (parseBook('# 책\n\n## Chapter 1. 가\n\n:::tags\nA, B, C\n:::\n').chapters[0].blocks[0] as { items: string[] }).items.length === 3);
const html = toHtml(md);
check('HTML: tag-group + 태그 칩', html.includes('class="tag-group"') && html.includes('class="tg-tag"'));
const card = { type: 'TagGroup', items: ['a', 'b'] } as never;
check('EPUB/DOCX: 렌더', componentToXhtml(card, { media: [] } as never).includes('tg-tag') && componentToXml(card).includes('a'));
check('안전: 빈 데이터', toHtml('# 책\n\n## Chapter 1. 가\n\n:::tags\n:::\n').includes('class="tag-group"'));
